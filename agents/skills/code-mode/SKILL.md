---
name: Code Mode
description: Use when task is specifically focused on writing new code (new feature, spec implementation, bugfix)
---
...
Default to working in the main session. Use subagents only when the user explicitly requests delegation or parallel agent work. When requested, give each agent a bounded task and only the context it needs. Do not delegate synthesis or spawn nested agents unless requested. Batch independent tool calls when useful.
...
- Changes to public interfaces, ownership, or module boundaries -> the **architect** skill. Routine function edits do not need a design workflow.
- Contested design -> compare alternatives against concrete requirements and evidence in the main session.
...
- **Guard the Context Window** (**principle-guard-the-context-window**). Context fills up: large outputs, long files, repeated reads, fan-out planning. Filter bulk output with searches and scripts; keep only relevant excerpts in context.
